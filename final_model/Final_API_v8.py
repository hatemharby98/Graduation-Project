...
def preprocess_multimodal_pipeline(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'is_nontcp' in df.columns and 'dst_port_is_wellknown' in df.columns:
        df['non_tcp_wellknown'] = df['is_nontcp'] * df['dst_port_is_wellknown']
        
    if 'bidirectional_duration_ms' in df.columns and 'bidirectional_mean_piat_ms' in df.columns:
        df['slow_signal'] = (
            np.log1p(df['bidirectional_duration_ms']) *
            np.log1p(df['bidirectional_mean_piat_ms'])
        )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ARTIFACTS = joblib.load('multimodal_inference_artifacts_v8.pkl')
    except Exception as e:
        raise RuntimeError(f"Failed to load artifacts: {e}")
        
    print("Loading Keras model...")
    try:
        MODEL = tf.keras.models.load_model('multimodal_ids_2head_model_v8.keras')
    except Exception as e:
...
class PredictionResult(BaseModel):
    ...
    is_anomaly: bool
    anomaly_score: float  # <--- Renamed to match your results.append logic
...
async def predict(request: PredictionRequest):
    ...
    for i in range(len(request.flows)):
        predicted_label = str(predicted_labels[i]) # Force string
        anomaly_p = float(anomaly_probs[i][0])     # Force float
        conf = float(class_probs[i, predicted_class_indices[i]]) # Force float
        
        # Enforce your strict logic
        if predicted_label == "Benign":
            is_anom = False
            final_anomaly_score = 0.0
        else:
            is_anom = bool(anomaly_p > 0.5)
            final_anomaly_score = float(round(anomaly_p * 100, 2))
            
        results.append(PredictionResult(
            predicted_class=predicted_label,
            class_confidence=float(round(conf * 100, 2)),
            is_anomaly=is_anom,
